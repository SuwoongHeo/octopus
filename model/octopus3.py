...
class Octopus(object):
    def __init__(self, num=8, img_size=1080):
        ...
        # In the paper, there are 7 Losses
        # L_s pose independent per-vertex loss || target_reg (T-pose) - smpld ||^2
        # L_s^ undressed body per-vertex loss || target_reg(wo_d) - smpl ||^2
        # L_N3D pose dep. per-vertex loss || target_reg - smpl ||^2
        # L_N2D sillouhette overlap loss || rendering - mask ||^2
        # L_J3D 3D Joint loss || target_joint - Joint 25 ||^2
        # L_theta Pose specific loss || R(target) - pose smpl||^2 + ||transl(target) - transl_smpl||^2
        # L_J2D 3D Joint projection loss ||Joint2D (target) - proj(joint(pred))||^2

        # Train procedure :
        # 1. Train pose batch with L_J3D, L_theta
        # 2. Train pose batch again with L_N3D, L_theta
        # 3. End-to-end train with alternating full supervision vs weak supervision
        # Full : L_s, L_s^, L_N3D, L_J3D / Weak : L_N2D, L_J2D
