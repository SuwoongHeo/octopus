...
def sparse_to_tensor(x):
    ...
    out = tf.sparse.reorder(tf.SparseTensor(indices, coo.data, coo.shape))
    return out
...
def batch_laplacian(v, f):
    ...
    cot_a = -1 * tf.reduce_sum(ab * ca, axis=2) / tf.sqrt(tf.reduce_sum(tf.linalg.cross(ab, ca) ** 2, axis=-1))
    cot_b = -1 * tf.reduce_sum(bc * ab, axis=2) / tf.sqrt(tf.reduce_sum(tf.linalg.cross(bc, ab) ** 2, axis=-1))
    cot_c = -1 * tf.reduce_sum(ca * bc, axis=2) / tf.sqrt(tf.reduce_sum(tf.linalg.cross(ca, bc) ** 2, axis=-1))
    ...
    sp_L_raw = [tf.sparse.reorder(tf.SparseTensor(l_indices[i], tf.reshape(W[:, i], (-1,)), shape)) for i in range(6)]
    ...
    dia = tf.sparse.reorder(tf.SparseTensor(tf.cast(indices, tf.int64), tf.reshape(dia_values, (-1,)), shape))
